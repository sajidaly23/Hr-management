...
const Sidebar = () => {
  ...
  const adminMenuItems = [
    ...
    { 
      href: '/addmin/empolyeeregisterpage', 
      label: 'Add Members', 
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <aside className="w-72 bg-gradient-to-b from-slate-950 via-slate-900 to-purple-950 shadow-2xl min-h-screen p-6 flex flex-col">
      ...
      <nav className="flex-1">
        <ul className="space-y-2">
          {menuItems.map((item) => {
            const isActive = pathname?.includes(item.href.replace('/addmin/', '').replace('/empolyee/', ''))
            return (
              <li key={item.href}>
                <Link 
                  href={item.href}
